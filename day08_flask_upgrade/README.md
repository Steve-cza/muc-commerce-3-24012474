...
- `陈子昂 8-1`：完成`/api/metrics`指标JSON接口；
- `陈子昂 8-2`：完成`/api/categories`的查询参数筛选；
- `陈子昂 8-3`：统一400错误JSON结构；
- `陈子昂 8-4`：检查数据服务返回值可被`jsonify`序列化；
- 为新增接口编写至少3条Flask测试。
...
- 姓名：陈子昂
- 学号：24012474
- 已完成路由或接口：/health, /api/metrics, /api/categories, /api/categories?category=Fashion, 统一错误处理
- 测试文件：tests/test_api.py（9项测试全部通过）
- 尚未解决的问题：无
